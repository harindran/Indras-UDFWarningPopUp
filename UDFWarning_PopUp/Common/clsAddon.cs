...
namespace UDFWarning_PopUp.Common
{
    class clsAddon
    {
        ...
        bool udfSelection = false,PopUpShown=false;
        ...
        private void objapplication_ItemEvent(string FormUID, ref SAPbouiCOM.ItemEvent pVal, out bool BubbleEvent)  
        {
            ...
            try
            {
                ...
                {
                    {
                        ...
                        switch (pVal.EventType)
                        {
                            case SAPbouiCOM.BoEventTypes.et_CLICK:
                                if (pVal.FormTypeEx == "139" && (pVal.ItemUID=="1" || pVal.ItemUID== "2349990001") && objform.Mode== SAPbouiCOM.BoFormMode.fm_ADD_MODE) //Sales Order
                                {                                    
                                    udfForm = objapplication.Forms.Item(objform.UDFFormUID);
                                    comboBox = (SAPbouiCOM.ComboBox)udfForm.Items.Item("U_FrtType").Specific;
                                    if (comboBox.Selected.Value == "0")
                                    {
                                        clsModule.objaddon.objapplication.MessageBox("Please select the Freight Type in User-Defined Fields!!!", 1, "OK");
                                        comboBox.Item.Click();                                        
                                        BubbleEvent = false;
                                    }
                                    else
                                    {
                                        if (udfSelection == false) { BubbleEvent = false; clsModule.objaddon.objapplication.MessageBox("Please select the Freight Type", 0, "OK"); return; }
                                    }
                                }
                                break;
                            case SAPbouiCOM.BoEventTypes.et_COMBO_SELECT:
                                if (pVal.FormTypeEx == "-139" && pVal.ItemUID == "U_FrtType" && (objform.Mode == SAPbouiCOM.BoFormMode.fm_OK_MODE || objform.Mode == SAPbouiCOM.BoFormMode.fm_UPDATE_MODE))
                                {
                                    BubbleEvent = false;
                                }
                                    break;
                ...
                else
                {
                    ...
                    {
                        case SAPbouiCOM.BoEventTypes.et_CLICK:
                            if (pVal.FormTypeEx == "139" && pVal.ItemUID=="4") //Sales Order
                            {
                                //if (clsModule.objaddon.objapplication.Menus.Item("6913").Checked == false) { clsModule.objaddon.objapplication.SendKeys("^+U"); }

                            }
                            break;
                        case SAPbouiCOM.BoEventTypes.et_FORM_LOAD:
                            if (pVal.FormTypeEx == "139") //Sales Order
                            {
                                if (clsModule.objaddon.objapplication.Menus.Item("6913").Checked == false) { clsModule.objaddon.objapplication.SendKeys("^+(U)"); }

                            }
                            if (pVal.FormTypeEx == "0" && PopUpShown==true)
                            {
                                SAPbouiCOM.StaticText staticText= (SAPbouiCOM.StaticText)objform.Items.Item("7").Specific;
                                //staticText.Item.TextStyle = 1;  //Bold Text  
                                staticText.Item.TextStyle =Convert.ToInt32(SAPbouiCOM.BoTextStyle.ts_BOLD);//Bold Text  
                                PopUpShown = false;
                            }
                            break;
                        case SAPbouiCOM.BoEventTypes.et_COMBO_SELECT:
                            if (pVal.FormTypeEx == "-139" && pVal.ItemUID == "U_FrtType" && objform.Mode == SAPbouiCOM.BoFormMode.fm_ADD_MODE)
                            {
                                
                                udfForm = objapplication.Forms.Item(FormUID);
                                comboBox = (SAPbouiCOM.ComboBox)udfForm.Items.Item("U_FrtType").Specific;                               
                                if (comboBox.Selected.Value != "0")
                                {
                                    PopUpShown = true;
                                    if (clsModule.objaddon.objapplication.MessageBox("You have selected \"" + comboBox.Selected.Description + "\", Are you Sure?", 2, "Yes", "No") != 1) { udfSelection = false; comboBox.Select(0, SAPbouiCOM.BoSearchKey.psk_Index); return; }
                                }                                    
                                udfSelection = true;
                            }
                            break;
                        case SAPbouiCOM.BoEventTypes.et_ITEM_PRESSED:
                            if (pVal.FormTypeEx == "139" && (pVal.ItemUID == "1" || pVal.ItemUID == "2349990001") && pVal.ActionSuccess==true && objform.Mode == SAPbouiCOM.BoFormMode.fm_ADD_MODE) //Sales Order
                            {
                                udfSelection = false;
                            }
                                break;
        ...
        private void DisConnect_Addon(bool addonform)
        {
            try
            {
                if (clsModule.objaddon.objapplication.Forms.Count > 0 && addonform==true)
                {
                    try
                    {
